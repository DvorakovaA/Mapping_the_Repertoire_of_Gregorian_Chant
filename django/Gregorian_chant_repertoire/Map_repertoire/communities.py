    ...
    if feast_ids == ['All']:
        ...
        for source_id in drupals:
            if DO_FILTER_OFFICE:
                ...
                chants_of_source = [chant[0] for chant in Data_Chant.objects.filter(source_id = source_id[0]).values_list('cantus_id')]  #Data_Chant.objects.filter(source_id = source_id[0]).values_list('cantus_id')[0] #
            if chants_of_source != []:
    ...
    if metric == 'Jaccard':
        for s_i in used_sources:
            for s_j in used_sources:
                distance_matrix[source_dict[s_i], source_dict[s_j]] = 1 - Jaccard_metrics(source_chants_dict[s_i], source_chants_dict[s_j])
